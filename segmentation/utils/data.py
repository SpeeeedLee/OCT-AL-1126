...
def data_loader(opath, fold):
    ...
    INPUT:
        ...
        fold: select the fold for cross-validation
...
def o_data(opath, batch_num, width, height):
    ...
    OUTPUT:
        img_sub: OCT images batch - shape: [batch_size, 1, width, height]
    """
...
def g_data_cell_binary(gpath, batch_num, width, height):
    """
    Reading cell nuclei labeling for BINARY segmentation
    ...
    OUTPUT:
        img_sub: binary nuclei mask batch - shape: [batch_size, 1, width, height]
                 Values: 0.0 (background) or 1.0 (nuclei)
    """
    NUM = len(batch_num)
    img_sub = np.zeros((NUM, 1, width, height), dtype=np.float32)
    
    i = 0
    for name in np.array(batch_num):
        # Read cell nuclei mask (assuming white pixels = nuclei)
        img = cv2.imread(gpath + name, 0)
        # Normalize to binary: 0 or 1
        img_binary = (img > 127).astype(np.float32)  # Threshold at 127
        img_sub[i, 0, :, 6:506] = img_binary
        i += 1
...
def g_data_cell(gpath, batch_num, width, height, USE_DS):
    """
    [DEPRECATED] Reading cell labeling for deep feature sharing model
    Use g_data_cell_binary() for binary segmentation instead.
    """
    NUM = len(batch_num)
...
def g_data_line(gpath, batch_num, width, height):
    """
    [DEPRECATED] Reading layers labeling for deep feature sharing model
    Not used in binary nuclei segmentation.
    """
    NUM = len(batch_num)
    img_sub = np.zeros((NUM, 1, width, height))
...
def g_data_all(gpath_cell, gpath_line, batch_num, width, height, USE_DS):
    """
    [DEPRECATED] Reading labeling for non deep feature sharing model (multi-class)
    Not used in binary nuclei segmentation.
    """
