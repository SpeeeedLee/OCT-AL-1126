#!/usr/bin/env bash
# 5.3.3　把 Core-set AL 軌跡延伸跑到 ρ=90%（seed 42），取得 bottom-10% leftover。
# 標準 4.4 協定（sweep lr + best-val）；同一 GPU 並行訓多個 lr（--parallel_lr）；結果寫隔離樹。
#   DEVICE=cuda:9 ./thesis/chapter_5/run_5_3_3_extend_coreset.sh
set -eu
cd "$(git rev-parse --show-toplevel)"
DEVICE="${DEVICE:-cuda:9}"
...
RESUME_SRC="${RESUME_SRC:-./classification/exp_results/classification_hard/AL_simclr/labeled_ids/coreset_seed42_bs16.json}"

echo "Core-set AL → ρ=90%  seed=42  device=$DEVICE  exp_path=$EXP  (resume@${RESUME_FROM}%)"
python3 classification/run_AL.py \
  --task_type hard --AL_strategy coreset \
