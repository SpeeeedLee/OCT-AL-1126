...
# (tree, label, color, marker, linestyle) — tree = exp_results/aug_curve/<tree>/.
# Colors match Ch4 (VF=teal, VF+HV=red as a 3x variant). All aug types drawn by
# default; trees with no data yet are silently skipped.
SERIES = [
    ("aug4",  "HF+VF+HVF (4x)", "#1F77B4", "o", "-"),
    ("vfhv3", "VF+HV (3x)",     "#D62728", "s", "-"),
    ("aug2",  "HF (2x)",        "#8C564B", "D", "-"),
    ("vf2",   "VF (2x)",        "#17BECF", "^", "-"),
    ("aug1",  "w/o Aug (1x)",   "#7F7F7F", "v", "-"),
]
...
def curve(tree):
    """{portion: (mean, std, n_seeds)} via per-seed best-lr for an aug tree
    (e.g. 'aug4', 'vf2', 'vfhv3'). Reads ONLY the 5-seed lr-swept data."""
    out = {}
    by_p = _collect(f"{EXP}/{tree}/nuclei/cold_start_random/random_*_bs8.json")
...
def main():
    ...
    any_data = False
    for tree, label, color, marker, ls in SERIES:
        c = curve(tree)
        if not c:
            print(f"  [skip] {tree}: no data"); continue
        ps = sorted(c)
        ...
        any_data = True
        print(f"  [ok] {tree}: " + "  ".join(f"{p:g}:{c[p][0]:.4f}±{c[p][1]:.4f}(n{c[p][2]})" for p in ps))
    if not any_data:
