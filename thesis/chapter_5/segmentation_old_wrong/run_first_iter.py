...
def parse_arguments():
    ...
    p.add_argument('--flip_set', type=str, default=None,
                   choices=['none', 'hf', 'vf', 'hfvf', 'vfhv', '4x'],
                   help='named flip-set, overrides --aug_factor flips '
                        '(vf=VF only 2x, vfhv=VF+HV 3x)')
    p.add_argument('--init', type=str, default='random', choices=['random', 'simclr', 'ae'],
                   help='encoder init (simclr/ae require --simclr_path; ae = autoencoder '
                        'encoder, loaded identically via load_state_dict strict=False)')
