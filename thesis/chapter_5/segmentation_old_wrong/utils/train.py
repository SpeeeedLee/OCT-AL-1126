...
def train_unet(label_idx, opt, device):
    ...
    # Build augmented training items (named flip-set overrides aug_factor if given)
    flip_set = getattr(opt, "flip_set", None)
    if flip_set:
        from thesis.chapter_5.segmentation.utils.data import FLIP_SETS_NAMED
        train_items = expand_with_aug(list(label_idx), flips=FLIP_SETS_NAMED[flip_set])
    else:
