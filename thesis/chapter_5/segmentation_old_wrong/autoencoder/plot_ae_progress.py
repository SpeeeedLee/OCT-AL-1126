...
def loss_curve(out):
    ...
    ax.plot(ep, mse, color="#1F77B4", lw=2.2)
    SNAP = {1, 3, 10, 30, 50, 100, 200, 500, 1000, 1500, 2000}
    sx = [e for e in ep if e in SNAP]; sy = [mse[ep.index(e)] for e in sx]
    ax.scatter(sx, sy, color="#D62728", zorder=5, s=45, label="snapshot epochs")
    ax.set_yscale("log")
    ax.set_xlabel("Epoch", fontsize=14)
    ax.set_ylabel("Reconstruction MSE  (log scale)", fontsize=14)
    ax.set_title(f"Autoencoder training loss  —  up to epoch {max(ep)} "
                 f"(latest MSE = {mse[-1]:.5f})", fontsize=14)
...
def main():
    ...
    ap.add_argument("--dataroot", default="./ds/segmentation")
    a = ap.parse_args()
