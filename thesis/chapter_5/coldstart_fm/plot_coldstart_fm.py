...
FAMILIES = [
    ...
    ("CLIP",                    [("clip:base",                 "Base",   "#BCBDDC"),
                                 ("clip:large",                "Large",  "#6A51A3")]),
    ("DINOv2",                  [("dinov2:small",              "Small",  "#A1D99B"),
                                 ("dinov2:base",               "Base",   "#41AB5D"),
                                 ("dinov2:large",              "Large",  "#006D2C")]),
    # ── Medical Image Models ─────────────────────────────────────────────────
    ("ResNet\n(SimCLR; Ours)",  [("simclr:resnet18_random",   r"18 ($\theta^1$)",  "#FCE38A"),
                                 ("simclr:resnet18",          r"18 ($\theta^2$)",  "#F4C430")]),
    ("ResNet\n(RadImageNet)",   [("radimagenet:resnet50",     "50",  "#E6550D")]),
    ("BiomedCLIP",              [("biomedclip:base",          "",    "#1FA191")]),
    ("RETFound",                [("retfound:oct",             "",    "#CB181D")]),
    ("MedImageInsight",         [("medimageinsight:base",     "",    "#8C6D31")]),
]

_FIRST_MEDICAL = "ResNet\n(SimCLR; Ours)"
...
def draw(portion, data, rb, out, y_lim=None):
    ...
    # y-limits: per-portion optimal range, always tick at interval 5, integers only
    ...
    TOKEN_FRAC = 0.12                             # label sits near the bottom of each bar
    _rand = rb.get(portion)
    _rand_top = _rand[0] if _rand else (y0 + (y1 - y0) * 0.25)
    TOKEN_Y = y0 + (_rand_top - y0) * TOKEN_FRAC
    for xpos, mean, std, color, size, multi, hatch in bars:
        # accuracy value: place above error bar top with a small gap
        val_y = mean + std + (y1 - y0) * 0.018
        ax.text(xpos, val_y, f"{mean:.1f}", ha="center",
                va="bottom", fontsize=FONT_TICK - 6, color="#222222")
        if size:                                  # VERTICAL token near bar bottom
            ax.text(xpos, TOKEN_Y, size, ha="center", va="bottom",
                    fontsize=FONT_TICK + 2, fontweight="bold",
    ...
    # vertical separator between Natural and Medical model groups
    sep_x = None
    for i, (disp, cx) in enumerate(fam_ticks):
        if disp == _FIRST_MEDICAL and i > 0:
            sep_x = (fam_ticks[i - 1][1] + cx) / 2
            break
    if sep_x is not None:
        ax.axvline(sep_x, color="black", linewidth=2.0, linestyle="--", alpha=0.85, zorder=2)
        ytext = y0 + (y1 - y0) * 0.97
        # "Natural Domain" at top-left of natural section (left axis edge)
        ax.text(ax.get_xlim()[0] + 0.1, ytext, "Natural Domain", ha="left", va="top",
                fontsize=FONT_TICK + 2, color="black", fontweight="bold", style="italic")
        # "Medical Domain" at top-left of medical section (just right of separator)
        ax.text(sep_x + 0.2, ytext, "Medical Domain", ha="left", va="top",
                fontsize=FONT_TICK + 2, color="black", fontweight="bold", style="italic")

    # Random reference line across the whole axis
    rbv = rb.get(portion)
    if rbv:
        ax.axhline(rbv[0], color="#CC0000", linestyle=(0, (6, 4)), linewidth=2.5, alpha=0.85, zorder=10)
    ...
    ax.legend(handles=[Patch(facecolor="white", edgecolor="black", hatch=_HATCH_CNN, label="CNN-based"),
              ...
              loc="upper right", fontsize=FONT_LEGEND, framealpha=0.9, ncol=1,
