...
def build_heatmap_matrix(
    ...
    For each SimCLR_BS x SimCLR_EP cell:
        ...
        1. For each seed, read that seed's JSON.
        2. Within that seed and portion, choose:
              - the best downstream LR by mean accuracy, if only_lr is None; or
              - the fixed downstream LR, if only_lr is provided.
        3. Take the mean accuracy under that selected LR for that seed.
        4. Average these per-seed selected-LR means across seeds.
    ...
    It does NOT pool all LR values across seeds.
    """
    ...
    if not filtered:
        raise RuntimeError(
            "No SimCLR JSON records matched the requested seeds/suffix/simclr_lr."
        )
    ...
    # -------------------------------------------------------------------------
    # Compute each cell:
    # per seed -> selected LR mean -> mean across seeds
    # -------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot SimCLR batch-size × epoch heatmap for one data portion."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0002",
        help="SimCLR pretraining LR to include in the heatmap.",
    )
    ...
    simclr_lr_tag = str(args.simclr_lr).replace(".", "p")
    aug_tag = args.aug_key

    # if args.only_lr is None:
    #     lr_tag = "bestlr"
    # else:
