...
def get_sample_ids(feature_path, percent=2, temperature=0.07, lr=0.001, max_iter=300, 
                  init='random', distance='euclidean', scheduler='none', balance=1.0):
    """
    直接獲取sample_ids的函數
    ...
    Args:
        feature_path: 特徵文件路徑
        percent: 採樣百分比
        temperature: softmax溫度參數
        lr: 學習率
        max_iter: 最大迭代次數
        init: 初始化方法 ('random' 或 'fps')
        distance: 距離類型 ('euclidean' 或 'cosine')
        scheduler: 調度器類型
        balance: 平衡比率
    ...
    # 加載數據
    input_data = np.load(feature_path)
    features, _ = input_data[:, :-1], input_data[:, -1]
    features = torch.Tensor(features).cuda()
    
    # 計算採樣數量
    total_num = features.shape[0]
    sample_num = round(total_num * percent * 0.01)
...
def main(args):
    ...
    total_num = features.shape[0]
    sample_num = round(total_num * args.percent * 0.01)
    print(sample_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_path', default='./Classification/Active_FT/features_resnet18_simclr.npy', type=str,
                        help='path of saved features')
    parser.add_argument('--output_dir', default='./Classification/Active_FT/features', type=str, help='dir to save the visualization')
